...
function swapIndex(array, indexA, indexB) {
  ...
  function bubbleSort(array) {
    const willResult = array.slice(); // 配列の各値をコピー
    for (let i = array.length - 1; i > 0; i--) {
      for (let j = 0; j < i; j++) {
        if(willResult[j]>willResult[j+1]) willResult = swapIndex(willResult, j, j + 1);
      }
    }
  }
  ...
  document.write(`Input Array(should not be sorted): [${unsortedArray.join(", ")}]`);