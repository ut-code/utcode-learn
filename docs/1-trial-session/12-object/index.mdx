...
次のような情報を持つ車を表すオブジェクト`car`を作成してください。
...
<ViewSource url={import.meta.url} path="_samples/create-car-object" />
...
次のような車を表すオブジェクト`car`があります。車の値段を表示し、その値段を3000000円にしてから、もう一度表示してください。
...
次のような車を表すオブジェクト`car`があります。所有者のメールアドレスを変更することになりました。所有者のメールアドレスを表示し、`"yamada.hanako@example.jp"`に変更してから、もう一度表示してください。
...
<ViewSource url={import.meta.url} path="_samples/change-email-property" />
