...
:::tip[`Hello World!` を表示する場所]

`button` 要素の `textContent` を書き換えるとボタンの中に `Hello World!` を表示してしまいます。

ボタンとは別に、あらかじめ `Hello World!` を表示するための場所を用意しておく必要があります。
例えば空の `div` 要素を置いておけばよいでしょう。
