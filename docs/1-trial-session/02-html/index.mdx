...
私たちは日々、[[ウェブブラウザ]]を通してさまざまなページやアプリを利用しています。これらはどのようにして作られているのでしょうか？実は、[[**HTML**]]と[[CSS]]、そして[[JavaScript]]の3つの言語を用いることで作ることができます。これらは互いに異なる役割をもっています。
...
まず、[[HTML]]を書くための[[ファイル]]を作成します。VS Codeの画面左端から、新しい[[ファイル]]を作成して、`index.html`と名付けましょう。次に、[[ファイル]]の中身を次のようにして、保存します。保存には[[ショートカットキー]]（<kbd>Control+S</kbd>（macOSでは<kbd>Command+S</kbd>））を用いてください。
...
プロジェクトフォルダを開き、`index.html`が作成されていることを確認しましょう。この[[ファイル]]をダブルクリックして開きます。`出かける前に、気象庁の天気予報を確認しましょう。`と表示されましたか？

<video src={openInBrowserVideo} controls />

:::info[拡張子]

VS Code上で作成した[[ファイル]]の名前は`index.html`でした。しかし、Windowsで[[フォルダ]]の中身を見ると`index`とだけ表示されることがあります。なぜでしょうか？

これは、Windowsでは[[拡張子]]が標準で表示されないためです。ファイル名の`.`（ピリオド）以降の部分は[[**拡張子**]]と呼ばれ、[[ファイル]]の種類を識別するために用いられることが多いです。たとえば、`index.html`の`html`が拡張子です。

下の動画を参考に、[[拡張子]]を表示する設定に変更しておきましょう。
