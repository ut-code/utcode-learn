...
JavaScriptで利用することができる<Term>値</Term>の種類として、これまで<Term>数値</Term>と<Term>文字列</Term>、<Term>論理値</Term>を扱いました。<Term>数値</Term>や<Term>文字列</Term>、<Term>論理値</Term>などの「それ以上分解できない」<Term>値</Term>のことを**プリミティブ**と呼びます。JavaScriptには、この他にも、**オブジェクト**と呼ばれる<Term>値</Term>が存在します。<Term>プリミティブ</Term>でない<Term>値</Term>はすべてオブジェクトです。

![値の種類](./value-types-with-object.drawio.svg)

オブジェクトを用いると、関連する<Term>値</Term>をまとめて1つの<Term>値</Term>として扱うことができます。これにより、現実世界の事物をプログラム上でうまく表現することができます。

たとえば、学生を例に考えてみましょう。学生には、名前や年齢、メールアドレスなどの属性があります。これをオブジェクトとして表現すると、次のようになります。
...
次のような属性を持つオブジェクト`book`を作成してください。
...
次のような本を表すオブジェクト`book`があります。本の場所を2階のE列から1階のA列に変更することになりました。本の場所を表示し、1階のA列に変更してから、もう一度表示してください。
...
<ViewSource url={import.meta.url} path="_samples/change-location-property" />

</Answer>

## 配列とオブジェクト

さきほど<Term>プリミティブ</Term>でない<Term>値</Term>はすべてオブジェクトであると説明しました。<Term>配列</Term>はプリミティブではないので、オブジェクトの一種です。
