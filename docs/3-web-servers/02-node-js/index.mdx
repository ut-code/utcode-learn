...
nvm のインストールが終わったら、ターミナルに nvm を認識させるためにターミナルアプリケーションを再起動してください。
...
<Tabs groupId="os">
  <TabItem value="mac" label="macOS">
    ...
    touch ~/.zshrc
    ```
    </TabItem><TabItem value="win" label="Windows (WSL)">
...
を実行して WSL を退出した後、Windows 側で

```shell title="Command Prompt"
wsl --shutdown
```

を実行すると WSL をシャットダウンすることができます。

シャットダウンした後は、再度 WSL にサインインしてください。

</TabItem>
</Tabs>
---

続いて、nvm のインストールが成功していることを確認するため、次のコマンドを実行します。
...
完了したら、 nvm インストール時と同様に、ターミナルを再起動してください。

Node.js のインストールが完了しているか確認するため、次のコマンドを実行します。
...
<Tabs groupId="os">
  ...
  <TabItem value="win" label="Windows (WSL)">
    Windows での作業は、WSL のターミナルを利用するようにしましょう。
...
:::tip `'MODULE_NOT_FOUND'`
`'MODULE_NOT_FOUND'` というエラーが出る場合、開いているディレクトリが違う可能性があります。

`pwd` や `ls` で、正しいディレクトリにいることを確認してください。
:::
...
Node.js のデバッグを開始するには、ブレークポイント等を設定したうえで、<kbd>F5</kbd> キー (または <kbd>Fn</kbd> + <kbd>F5</kbd>) を押します。初回はデバッグ構成を選択するメニューが出現するので、`Node.js` を選択しましょう。デバッグが開始されると、VS Code 下部の青いバーが橙色に変化します。`console.log` は `DEBUG CONSOLE` タブに出力されるので注意しましょう。

ステップ実行、コールスタックなど、基本的な機能はブラウザに搭載されているものとほとんど同じです。

:::warning
Node.js をインストールした直後は、「Node バイナリが見つかりません。」のようなエラーメッセージが出る場合があります。
VS Code を再起動 (macOS の場合は <kbd>command</kbd> + <kbd>Q</kbd>)しましょう。
:::
