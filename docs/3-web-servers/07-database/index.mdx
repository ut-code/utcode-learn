...
これまで作成してきたアプリケーションでは、次のように、データを全て Node.js 内の変数に記録していました。しかし、このような方法では、<Term>サーバー</Term>が終了するたびにデータが消えてしまいます。
...
- [**`@prisma/client` パッケージ**](https://www.npmjs.com/package/@prisma/client): アプリケーションサーバーからインポートして用いられる npm のパッケージ。JavaScript プログラムから使用する。
