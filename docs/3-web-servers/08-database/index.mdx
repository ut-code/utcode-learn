...
`.env`ファイルは、環境変数の指定を簡略化するために慣習的に用いられるファイルです。`node --env-file=.env main.mjs`のように指定することで、`.env`ファイルに記述された環境変数を読み込ませることができます。Drizzleは`.env`ファイルの内容を自動的には読み込まないため、Node.jsの実行時には`--env-file`オプションを指定する必要があります。ただし、`drizzle-kit`のコマンドは`.env`ファイルを自動的に読み込みます。
:::
