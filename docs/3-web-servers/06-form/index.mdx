...
フォームで送信された文字列を反転して返すサーバーを作ってみましょう。
...
JavaScript で文字列を反転するには、 `String#split` メソッドと `Array#reverse` メソッドが使えます。

ここでは重要ではないので、サンプルコードを載せておきます。

```js title="main.mjs"
const original = "!dlroW olleH";
...
<head>
  <meta charset="utf-8" />
  <title>文字列反転フォームの回答例</title>
</head>
<body>
<form action="reverse">
  <input name="textToReverse">
  <button type="submit">反転する</button>
</form>
</html>
```
