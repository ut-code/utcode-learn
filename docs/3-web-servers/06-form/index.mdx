...
[npm の章](/docs/web-servers/module/#node-emoji-%E3%83%91%E3%83%83%E3%82%B1%E3%83%BC%E3%82%B8)で扱った `node-emoji` パッケージの `emojify` 関数を送られた文字列に適用するサーバーを作ってみましょう。
...
<Answer title="node-emoji フォーム">

`npm install node-emoji express` とするのを忘れないようにしましょう。
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>node-emoji フォームの解答例</title>
  </head>
