...
`String#replace` と `fs.readFileSync` 関数を用いることで、HTML を別のファイルに分けて保存することができるようになり、より簡単に管理することができます。
...
その次の行の `index.replace` で、`index` の中の `"{users}"` が `namesAsHTML` に置換されます。
