...
1. [データベース](/docs/web-servers/database/)の節と同じように、データベースを作成し、Expressを用いてWebサーバーを作成します。
   ...
   1. `npm install @prisma/client @prisma/adapter-pg pg dotenv`コマンドと`npm install -D prisma @types/pg`コマンドを実行して、Prismaのセットアップに必要なパッケージをインストールします。
   1. `npx prisma init`コマンドを実行してPrismaのセットアップに必要なファイルを作成します。
   ...
   1. `npx prisma db push`コマンドを実行してテーブルとカラムの定義をデータベースに反映し、`npx prisma generate`コマンドを実行して`Prisma Client`クラスを生成します。
