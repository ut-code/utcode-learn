...
商品リストとショッピングカートを管理するアプリケーションを作成しましょう。以下の機能を実装してください。

**要件:**

- 商品リスト（商品名、価格、在庫数）を表示する
- 商品をカートに追加できる（在庫がある場合のみ）
- カート内の商品数量を増減できる
- カートから商品を削除できる
- カート内の合計金額を表示する
- 商品をカートに追加すると在庫が減る
- カートから削除・数量を減らすと在庫が戻る

**ヒント:**

- 商品の型を定義しましょう（例: `type Product = { id: number; name: string; price: number; stock: number }`）
- カート内のアイテムの型も定義しましょう（例: `type CartItem = { productId: number; quantity: number }`）
- 在庫数とカート内の数量の整合性を保つように状態を更新しましょう
- 合計金額は状態を基に計算できます
...
<ViewSource url={import.meta.url} path="_samples/shopping-cart" />
