...
`content` は主に `width` や `height` などの属性を、
`padding`, `border`, `margin` はそれぞれ主に `padding`, `border`, `margin` などの属性を操作することで制御できます。
...
グリッドを利用するには、配置したい要素の**親要素**に `display: grid` を適用し、`grid-template-columns` 属性で列の幅を、`grid-template-rows` 属性で行の幅を指定します。
...
`position` 属性を操作すると、要素の位置指定の基準を変更することができます。
...
`position` 属性を使うと、親要素を基準にして子要素の位置を指定することができます。
