...
上の解答例のようなコードでは、`person` に `age` や `name` 以外のプロパティがあった場合、戻り値のオブジェクトにそれらのプロパティが含まれません。
オブジェクトの全てのプロパティをコピーするには、[スプレッド構文](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Operators/Spread_syntax)というものを使います。
...
重複するプロパティを上書きする性質を利用して、このように書くこともできます。
