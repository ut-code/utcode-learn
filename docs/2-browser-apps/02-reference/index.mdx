...
上のコードでは、`person` の `age` と `name` 以外のプロパティが失われてしまいます。
オブジェクトの全てのプロパティをコピーするには、[スプレッド構文](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Operators/Spread_syntax)を使います。
