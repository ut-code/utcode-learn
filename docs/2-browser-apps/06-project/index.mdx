...
まずは、関数を分けて `onclick` を宣言的に書き直します。
...
addButton.onclick = () => {
  const todoItem = createTodo(todoInput.value).todoItem;
  todoInput.value = "";
  addButton.disabled = true;
  todoList.appendChild(todoItem);
};
...
function createTodo(text) {
  ...
  editButton.textContent = "編集";
  editButton.onclick = () => startEdit(self);
  deleteButton.textContent = "削除";
  deleteButton.onclick = deleteItem(self);
  todoItem.appendChild(todoText);
  todoItem.appendChild(editButton);
  todoItem.appendChild(deleteButton);
  return self;
...
function startEdit(self) {
  const { todoItem, todoText, editButton, deleteButton } = self;
  const input = document.createElement("input");
  const confirmButton = document.createElement("button");
  self.input = input;
  self.confirmButton = confirmButton;
  input.value = todoText.textContent;
  input.oninput = () => validate(self);
  confirmButton.textContent = "確定";
  confirmButton.onclick = () => endEdit(self);
  todoItem.replaceChild(input, todoText);
  todoItem.replaceChild(confirmButton, editButton);
}
function endEdit(self) {
  const { todoItem, todoText, input, editButton, confirmButton } = self;
  todoText.textContent = input.value;
  todoItem.replaceChild(todoText, input);
  todoItem.replaceChild(editButton, confirmButton);
}
...
function validate(self) {
  const { confirmButton, todoInput } = self;
  confirmButton.disabled = todoInput === "";
}
```

</Answer>

`self` が多く出てきて管理が面倒なので、クラスにまとめてしまいましょう。
クラスにすればメソッドとして自身のプロパティを使う関数を簡単に記述することができます。
