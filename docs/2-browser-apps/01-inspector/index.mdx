...
1. ブレークポイント  
   設置した行でプログラムの実行が一時停止するもので、Google Chromeの開発者ツールではSourcesタブからファイルを開くと表示される行番号をクリックすると設置できる。

2. ステップオーバーボタン (<VscDebugStepOver style={{ verticalAlign: "middle" }} />)  
   現在止まっている行で関数の処理が行われる場合は、その関数の内部の処理をすべて実行して次の行で止まるものです。

3. ステップインボタン (<VscDebugStepInto style={{ verticalAlign: "middle" }} />)  
   現在の行のプログラムを実行し、次の処理の行へ移動した後、プログラムが停止します。処理の流れを1個ずつ確認できます。

4. ステップアウトボタン (<VscDebugStepOut style={{ verticalAlign: "middle" }} />)  
   現在処理が止まっている関数内の残りの処理をすべて実行し、関数を呼び出している次の行へ移動します。
...
下の画像の中の、オレンジ色の線で挟まれている部分は、**これから実行されようとしている行**を表します。つまり、5 行目のプログラムは、この時点ではまだ実行されていません。

:::

![ブレークポイントの設置](./set-breakpoint.png)

続いて、**ステップ オーバー**ボタンを押します。ステップ オーバーは、現在の行を実行し、次の行に進む操作です。これにより、オレンジ色の線で挟まれた部分が 6 行目に移ります。
...
今度はブレークポイントを6行目に設置してみましょう。ページを更新して、次は、**ステップ イン**ボタンを押してみましょう。現在の行が関数呼び出しだった場合、実行が関数の中に移ります。
...
もう一度、**ステップ イン**ボタンを押してみましょう。そうすると、return文の処理が終わり、2 行目の末尾で実行が止まっているのが分かると思います。

![ステップ イン 2 回目](./step-in2.png)

ページを更新し、今度は、**ステップ イン**ボタンを押し、それから**ステップ アウト**ボタンを押しましょう。**ステップ アウト**ボタンにより、現在実行されている関数が最後まで実行され、6 行目の処理が全て終わったのが分かると思います。
