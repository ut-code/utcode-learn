...
重心の座標 (`center_pos`) プロパティと面積を求めるメソッドを持つ `Shape` クラスを実装してみましょう。

そして、 `Shape` クラスを継承する `Trapezoid`(台形) クラス、`Rectangle`(長方形) クラス、`Circle`(円) クラスを実装してみましょう。
...
`getArea()` 関数を頻繁に使うようなら、`area` をプロパティに保存しておいてもいいでしょう。
...
<ViewSource url={import.meta.url} path="_samples/class-shape-optimized" />
