...
ここまでクラスについて学んできましたが、クラスが使えると何が嬉しいかを確認しておきましょう。

- 関連するデータとメソッドをまとめ、関連する機能を持つ部分を一緒に扱うことができる
- 同じクラスから複数のオブジェクトを作成したり、継承を使って既存のクラスを拡張し、新しいクラスを作るなどして、コードを再利用できる
- 関連するコードがまとめられているため、修正や変更が必要な場合にも影響を受ける範囲が限定される

これによりプログラミングが効率的にできるようになります。ここにあげたのは最低限のメリットなので、他にどのようなメリットがあるか気になる人は自分で調べてみましょう。
