...
また、DOM を利用して作成または取得した HTML 要素を表すオブジェクトは、[`HTMLDivElement` クラス](https://developer.mozilla.org/ja/docs/Web/API/HTMLDivElement)を継承しています。
...
ドキュメントを読み、[`Date` クラス](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Date)を使って「その日の残り時間」を表示してみましょう。

例:

入力: 21時25分40秒に実行

出力: 今日は残り2時間 34分 20秒です。
...
document.write(
  `今日は残り${remainingHours}時間 ${remainingMinutes}分 ${remainingSeconds}秒です。`,
);
```
