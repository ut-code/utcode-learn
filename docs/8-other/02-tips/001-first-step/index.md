...
git add, git commit, git push origin branch名とし、pull requestを作成します。誰かにreviewをしてもらいましょう。vscodeのgit graphやsource controlの機能を使うと便利です。git pushをする前に、remoteの変更をpullしてmergeしておきましょう。
...
copilotの利用
慣れてきたら、copilotよりも通常の補完機能の方を使った方が開発のスピードが速くなります。
